...
function split(list4, index2) {
  return split_loop(list4, index2, toList([]));
}
...
// build/dev/javascript/sortable/sortable.mjs
var SortableConfig = class extends CustomType {
  constructor(on_reorder, container_id, item_class, dragging_class, drag_over_class, ghost_class) {
    super();
...
var NoDrag = class extends CustomType {
};
var Dragging = class extends CustomType {
  constructor(source_index, over_index) {
    super();
    this.source_index = source_index;
    this.over_index = over_index;
  }
};
var SortableItem = class extends CustomType {
  constructor(id2, data) {
    super();
    this.id = id2;
    this.data = data;
  }
};
var StartDrag = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
var DragEnter = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
};
var DragLeave = class extends CustomType {
};
...
var DragEnd = class extends CustomType {
};
var TouchStart = class extends CustomType {
  constructor($0, $1, $2) {
    super();
    this[0] = $0;
    this[1] = $1;
...
var TouchMove = class extends CustomType {
  constructor($0, $1) {
    super();
    this[0] = $0;
    this[1] = $1;
...
var TouchEnd = class extends CustomType {
};
...
function default_config(on_reorder, container_id) {
  return new SortableConfig(
    on_reorder,
    container_id,
    "sortable-item",
    "sortable-dragging",
...
function reorder_list(items, from_index, to_index) {
  let $ = from_index === to_index;
  if ($) {
    return items;
  } else {
    let item_count = length(items);
    let $1 = from_index >= 0 && from_index < item_count && to_index >= 0 && to_index < item_count;
    if ($1) {
      let $2 = split(items, from_index);
      let before_from = $2[0];
      let after_from = $2[1];
      if (after_from instanceof Empty) {
        ...
        let without_moving_item = append(before_from, rest_after_from);
        let $3 = split(without_moving_item, to_index);
        let before_to = $3[0];
        let after_to = $3[1];
        return append(before_to, prepend(moving_item, after_to));
      }
    } else {
      return items;
    }
...
function create_sortable_item(id2, data) {
  return new SortableItem(id2, data);
}
function get_item_data(item) {
  return item.data;
}
function drag_start_decoder(index2) {
  return success(new StartDrag(index2));
}
function drag_over_decoder() {
  return success(new DragOver(-1));
}
function drag_enter_decoder(index2) {
  return success(new DragEnter(index2));
}
function drag_leave_decoder() {
  return success(new DragLeave());
}
function drop_decoder(index2) {
  return success(new Drop(index2));
}
function drag_end_decoder() {
  return success(new DragEnd());
}
function touch_start_decoder(index2) {
  return success(new TouchStart(index2, 0, 0));
}
function touch_move_decoder() {
  return success(new TouchMove(0, 0));
}
function touch_end_decoder() {
  return success(new TouchEnd());
}
function render_sortable_item(config, drag_state, item, index2, render_item2) {
  let _block;
  if (drag_state instanceof NoDrag) {
    _block = false;
  } else {
    let source_index = drag_state.source_index;
    _block = source_index === index2;
  }
  let is_dragging = _block;
  let _block$1;
  if (drag_state instanceof Dragging) {
    let $ = drag_state.over_index;
    if ($ instanceof Some) {
      let source_index = drag_state.source_index;
      let over_index = $[0];
      _block$1 = over_index === index2 && source_index !== index2;
    } else {
      _block$1 = false;
    }
  } else {
  ...
  let item_classes = config.item_class + (() => {
    if (is_dragging) {
  ...
  return div(
    toList([
      ...
      attribute2("data-index", to_string(index2)),
      attribute2("data-item-id", item.id),
      style(
        "cursor",
        (() => {
          if (is_dragging) {
      ...
      on("dragstart", drag_start_decoder(index2)),
      (() => {
        let _pipe = on("dragover", drag_over_decoder());
        return prevent_default(_pipe);
      })(),
      (() => {
        let _pipe = on("dragenter", drag_enter_decoder(index2));
        return prevent_default(_pipe);
      })(),
      (() => {
        let _pipe = on("dragleave", drag_leave_decoder());
        return prevent_default(_pipe);
      })(),
      (() => {
        let _pipe = on("drop", drop_decoder(index2));
        return prevent_default(_pipe);
      })(),
      (() => {
        let _pipe = on("dragend", drag_end_decoder());
        return prevent_default(_pipe);
      })(),
      (() => {
        let _pipe = on("touchstart", touch_start_decoder(index2));
        return prevent_default(_pipe);
      })(),
      (() => {
        let _pipe = on("touchmove", touch_move_decoder());
        return prevent_default(_pipe);
      })(),
      (() => {
        let _pipe = on("touchend", touch_end_decoder());
        return prevent_default(_pipe);
      })()
    ]),
    toList([
      map3(
        render_item2(item, index2, drag_state),
        (var0) => {
          return new UserMsg(var0);
        }
      )
    ])
  );
}
function sortable_container(config, drag_state, items, render_item2) {
  let _block;
  if (drag_state instanceof NoDrag) {
    _block = "sortable-container";
  } else {
    _block = "sortable-container sortable-active";
  }
...
function update_sortable(sortable_msg, drag_state) {
  if (sortable_msg instanceof StartDrag) {
    let index2 = sortable_msg[0];
    return [new Dragging(index2, new None()), new None()];
    ...
    if (drag_state instanceof NoDrag) {
      ...
      let source_index = drag_state.source_index;
      return [new Dragging(source_index, new Some(index2)), new None()];
    }
  } else if (sortable_msg instanceof DragLeave) {
    return [drag_state, new None()];
  } else if (sortable_msg instanceof Drop) {
    let target_index = sortable_msg[0];
    if (drag_state instanceof NoDrag) {
      return [drag_state, new None()];
    } else {
      let source_index = drag_state.source_index;
      return [new NoDrag(), new Some([source_index, target_index])];
    }
...
// build/dev/javascript/examples/examples.mjs
var FILEPATH = "src/examples.gleam";
var Model = class extends CustomType {
  constructor(items, drag_state) {
    super();
    this.items = items;
    this.drag_state = drag_state;
  }
};
var SortableMsg = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function init(_) {
  return [
    new Model(
      toList(["\u{1F3AF} Item 1", "\u{1F680} Item 2", "\u2728 Item 3", "\u{1F3A8} Item 4", "\u{1F525} Item 5"]),
      new NoDrag()
    ),
    none()
  ];
...
function update2(model, msg) {
  if (msg instanceof SortableMsg) {
    let sortable_msg = msg[0];
    let $ = update_sortable(sortable_msg, model.drag_state);
    let new_drag_state;
    let maybe_reorder;
    new_drag_state = $[0];
    maybe_reorder = $[1];
    if (maybe_reorder instanceof Some) {
      let from_index = maybe_reorder[0][0];
      let to_index = maybe_reorder[0][1];
      let new_items = reorder_list(model.items, from_index, to_index);
      return [new Model(new_items, new_drag_state), none()];
    } else {
      return [new Model(model.items, new_drag_state), none()];
    }
  } else {
    return [model, none()];
  }
}
function render_item(item, index2, drag_state) {
  let _block;
  if (drag_state instanceof NoDrag) {
    _block = false;
  } else {
  ...
  let _block$2;
  if (is_dragging) {
    _block$2 = " opacity-50 scale-105 rotate-2 border-blue-500 bg-blue-50 z-10";
  } else {
    if (is_drag_over) {
      _block$2 = " border-green-400 bg-green-50 scale-105";
    } else {
      _block$2 = " border-gray-200 bg-white hover:border-blue-300 hover:shadow-lg";
    }
  }
  let drag_classes = _block$2;
  return div(
    toList([class$(base_classes + drag_classes)]),
    toList([
      div(
        toList([class$("flex items-center justify-between")]),
        toList([
          span(
            toList([class$("font-medium text-gray-800 text-lg")]),
            toList([text2(get_item_data(item))])
          ),
          div(
            ...
            toList([
              span(
                ...
                toList([text2("Index: " + to_string(index2))])
              ),
              (() => {
                if (is_dragging) {
                  return span(
                    toList([
                      class$(
                        "text-xs text-blue-600 font-semibold animate-pulse"
                      )
                    ]),
                    toList([text2("\u270B Dragging...")])
                  );
                } else {
                  if (is_drag_over) {
                    return span(
                      toList([
                        class$(
                          "text-xs text-green-600 font-semibold animate-bounce"
                        )
                      ]),
                      toList([text2("\u{1F4CD} Drop here!")])
                    );
                  } else {
                    return span(
                      toList([class$("text-xs text-gray-400")]),
                      toList([text2("\u{1F3AF} Drag me!")])
                    );
                  }
                }
              })()
...
function view(model) {
  let sortable_items = index_map(
    model.items,
    (item, index2) => {
      return create_sortable_item(
        "item-" + to_string(index2),
        item
      );
    }
  );
  let config = default_config(
    (_, _1) => {
      return new SortableMsg(new UserMsg(new NoOp()));
    },
    "sortable-container"
  );
  let _block;
  let $ = model.drag_state;
  if ($ instanceof NoDrag) {
    _block = "\u{1F3AF} Ready to drag";
  } else {
    let source = $.source_index;
    let target = $.over_index;
    _block = "\u{1F3AF} Dragging item " + to_string(source) + (() => {
      if (target instanceof Some) {
        let t = target[0];
        return " over item " + to_string(t);
      } else {
        return "...";
      }
    })();
  }
  let drag_status = _block;
  return div(
    toList([
      class$("min-h-screen bg-gradient-to-br from-purple-50 to-pink-50 p-8")
    ]),
    toList([
      div(
        toList([class$("max-w-md mx-auto")]),
        toList([
          div(
            toList([class$("text-center mb-8")]),
            toList([
              h1(
                toList([class$("text-3xl font-bold text-gray-900 mb-2")]),
                toList([text2("\u{1F680} Drag & Drop Demo")])
              ),
              p(
                toList([class$("text-gray-600 mb-4")]),
                toList([text2("Drag items to reorder them")])
              ),
              div(
                toList([
                  class$(
                ...
                toList([
                  p(
                    toList([class$("text-sm font-medium text-gray-700")]),
                    toList([text2("Status: " + drag_status)])
                  )
                ])
              )
            ])
          ...
          div(
            ...
            toList([
              h3(
                ...
                toList([text2("\u{1F4CB} Current Order")])
              ),
              div(
                    ...
                    return div(
                      ...
                      toList([
                        span(
                          ...
                          toList([
                            text2(to_string(index2 + 1) + ".")
                          ])
                        ),
                        span(
                          toList([class$("text-sm font-medium text-gray-800")]),
                          toList([text2(item)])
                        )
                      ])
                    );
          ...
          div(
            toList([class$("mt-6 text-center")]),
            toList([
              p(
                toList([class$("text-xs text-gray-500")]),
                toList([text2("\u2728 Pure Gleam drag & drop with Lustre")])
              )
            ])
          )
        ])
...
function main() {
  ...
  return console_log("\u{1F680} Sortable drag & drop example started!");
}
